...
import { CreditCard, Banknote, QrCode, Loader2, CheckCircle, Printer, X } from 'lucide-react';
              ...
              {PAYMENT_TYPES.map(pt => (
                <button
                  ...
                  className={clsx(
                    ...
                    paymentType === pt.key ? pt.color + ' border-2' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                  )}
                >
    ...
    <div className="space-y-5 animate-fadeIn">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{orders.length} ta to'lov kutilmoqda</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 font-semibold">To'lov kutilayotgan buyurtma yo'q</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {orders.map(order => {
            const total = order.items.reduce((s, i) => s + i.price * i.quantity, 0);
            return (
              <div key={order.id} className="card border-2 border-purple-100 hover:border-purple-300 transition-all cursor-pointer"
                onClick={() => setSelectedOrder(order)}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-bold text-gray-900 text-lg">{order.table_number}-stol</p>
                    <p className="text-xs text-gray-400">{formatDate(order.created_at)}</p>
                  </div>
                  <span className="badge bg-purple-100 text-purple-700">To'lov kutilmoqda</span>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  <span className="text-sm text-gray-500">{order.guest_count} mehmon</span>
                  <span className="font-bold text-green-600 text-lg">{formatPrice(total)}</span>
                </div>

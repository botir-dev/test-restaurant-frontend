...
import { orderApi, productApi } from '@/lib/services';
...
export default function AddToOrderPage() {
  ...
  const order = (ordersData?.data?.data || []).find((o: any) => o.id === orderId);
    ...
    mutationFn: () => {
      ...
      const newItems = cartItems.map(([pid, qty]) => {
        ...
        return {
          ...
          quantity: qty, is_prepared: false,
        };
      });
  ...
  return (
    <div className="space-y-4 animate-fadeIn pb-36">
      ...
      <div className="relative">
        ...
        <input className="input pl-9" placeholder="Mahsulot qidirish..."
        ...
        {types.map(type => (
          <button key={type} onClick={() => setActiveType(type)}
            className={clsx(
              ...
              activeType === type ? 'bg-green-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-green-300'
            )}>
                ...
                <div className="w-full h-20 bg-gray-100 rounded-xl mb-2 overflow-hidden">
                  {product.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <ShoppingBag className="w-6 h-6 text-gray-300" /></div>
                  )}
                </div>
        ...
        <div className="fixed bottom-4 left-4 right-4 lg:left-60 z-40 animate-slideUp">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="font-bold text-gray-900">{cartItems.length} xil qo'shildi</p>
              <p className="font-bold text-green-600">
                {formatPrice(cartItems.reduce((s, [pid, q]) => {
                  const p = products.find(p => p.id === pid);
            ...
            <button onClick={() => updateMutation.mutate()} disabled={updateMutation.isPending}
              ...
              {updateMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />}
              Buyurtmaga qo'shish
            </button>
          </div>
        </div>

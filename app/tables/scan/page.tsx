...
import { QrCode, Keyboard, ArrowLeft, Loader2, CheckCircle } from 'lucide-react';
...
export default function ScanPage() {
  ...
  const findAndOccupy = useMutation({
    mutationFn: async (id: string) => {
      ...
      const table = tables.find((t: any) => t.id === id || String(t.table_number) === id);
      if (!table) throw new Error('Stol topilmadi');
      if (table.is_occupied) throw new Error(`${table.table_number}-stol allaqachon band`);
    ...
    <div className="space-y-5 animate-fadeIn">
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-semibold transition-all ${
              mode === m.key ? 'bg-white text-green-700 shadow-sm' : 'text-gray-500'
            }`}
          >
      ...
      {mode === 'scan' ? (
        <div className="card text-center">
          ...
          <button onClick={() => setMode('manual')} className="btn-secondary justify-center text-sm">
        ...
        <div className="card">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <p className="font-semibold text-gray-900">Stol ID yoki raqamini kiriting</p>
              <p className="text-xs text-gray-500">QR koddan skanerlangan ID yoki stol raqami</p>
            </div>
          </div>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            >
              ...
              {findAndOccupy.isPending ? 'Tekshirilmoqda...' : scanned ? 'Topildi!' : 'Stolni topish'}
            </button>
          </form>
        </div>
      )}
      ...
      <div className="card bg-amber-50 border-amber-200">
        <p className="text-sm text-amber-800 font-semibold mb-1">📋 Qanday ishlaydi?</p>
